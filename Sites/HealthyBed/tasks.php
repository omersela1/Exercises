...
<body>
    ...
    <div id="wrapper">
        <?php
            ...
            echo '<br><div id="addButton" class="btn searchButton addremoveButton">Add Task To List</div><br>
            <form id="addForm" action="admin.php?task='.$_GET["id"].'" method="post">
                <label>Bed #: </label><input type="text" name="newTaskBedId" class="form-control" placeholder="Bed #'.$_GET["id"].'" value='.$_GET["id"].'>
                <label>Task Type: </label><select name="newTaskName" class="form-control" placeholder="Task Type" value="">
            ...
            $tasksQuery = 'select * from tbl_217_tasks where bed_id=' .$_GET["id"]. ' order by time_added desc';
            $taskList = mysqli_query($connection, $tasksQuery);
            if (mysqli_num_rows($taskList) != 0) {
                while ($row = mysqli_fetch_assoc($taskList)) {
                    echo '<div class="updateObject"><h2>'.$row["task_name"].'</h2><br>
                    <h5>Task Status: '.$row["task_status"].'</h5>
                    <h5>Time Added: '.$row["time_added"].'</h5>
                    <h5>Time Performed: '.$row["time_performed"].'</h5>';
                    if ($row["task_status"] == "Active")
                        echo '<div class="updateButtons"><a href="admin.php?taskCompleted='.$row["task_id"].'&bed='.$_GET["id"].'">Perform Task</a></div></div>';
                    else
                        echo '</div>';
                }
            }
            else
            echo '<p>The task list for this bed is currently empty.</p>';
        ?>
